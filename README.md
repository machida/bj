...
**「Ruby で書いたロジックを、CRuby なしでブラウザ上で動かせるか？」** を検証したプロジェクトです。

[Spinel](https://github.com/matz/spinel) の `-S` フラグが C コードを stdout に出力することに気づき、
その C を Emscripten に渡すだけで WASM になる——という一本道のパイプラインを確かめたくなりました。

ブラックジャックを選んだのは、カード操作・乱数・状態管理・勝敗判定・UI 連携を
コンパクトに含む題材として、パイプライン全体の動作確認に程よい複雑さだったためです。
`Fiber` や多値返却など Spinel が未対応の機能を避けながら Ruby らしいコードを書けるかも、
副次的な検証点でした。
